...
import beethovenImage from '../assets/media/Beethoven.jpg'; 

const BeethovenCard = ({ learnMoreUrl = "https://en.wikipedia.org/wiki/Ludwig_van_Beethoven", avatarImage = beethovenImage }) => {
  const synthRef = useRef(window.speechSynthesis);

  const person = {
    name: "Ludwig van Beethoven",
    challenge: "Deafness",
    achievement: "Legendary Composer",
    description:
      "Despite losing his hearing, Beethoven composed some of the most iconic and influential pieces in classical music history, proving that determination can defy any limitation."
  };
          ...
          <div className="flex items-start gap-6 mb-6">
            <div className="relative">
              <div className="bg-gradient-to-br from-purple-400/30 to-blue-400/30 border-2 border-dashed border-white/40 rounded-2xl w-20 h-20 flex items-center justify-center group-hover:border-yellow-400/60 transition-colors duration-300 overflow-hidden">
                <img src={avatarImage} alt={person.name} className="w-full h-full object-cover rounded-xl" />
              </div>
...
BeethovenCard.propTypes = {
  learnMoreUrl: PropTypes.string,
  avatarImage: PropTypes.string,
};

export default BeethovenCard;
